...
function findserver() {
  ...
  if [ -e ~/donjon/ansible ]; then
    ...
    cmd="$mount git-crypt unlock; chmod 0600 ~/donjon/????/??????????????.key; ssh-add ~/donjon/????/??????????????.key"
  fi
  if [ -n "$cmd" ]; then
    BUFFER=$cmd
    zle .accept-line
...
export PROMOTE_DEV_DISABLE_LOGRAGE=1
export SELENIUM_APP_HOST=http://0.0.0.0:4000 SELENIUM_HOST=localhost 
